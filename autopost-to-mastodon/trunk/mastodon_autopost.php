...
class autopostToMastodon
{
	...
	public function toot_post( $id ) {
		...
		if ( $toot_on_mastodon_option ) {
			...
			$message = stripslashes($_POST['autopostToMastodon_toot']);
	...
	public function add_metabox() {
		add_meta_box(
			...
			__( 'Toot editor', 'autopost-to-mastodon' ),
			array($this, 'metabox'),
			['post', 'page'],
			'side',
			'high'
	...
	public function metabox( $post ) {
		$status = get_post_meta( $post->ID, 'autopostToMastodon-post-status', true );
	...
	private function sendTestToot(){
		...
		$message=__("This is my first post with Mastodon Autopost",'autopost-to-mastodon');
		$media=null;
		$toot = $client->postStatus($message, $mode, $media);
